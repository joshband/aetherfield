...
- **docs/phase1-s1-plan.md** fixes the render-thread contract style — `prepare` off the render thread and the only allocating operation; `reset` and `process` noexcept, allocation-free and bounded; caller-owned signal buffers — which any transport mechanism decided here must fit without modification.
...
ADR-003 restated both deferrals verbatim ("Still DEFERRED, unchanged: `N`, the delay lengths `mᵢ`"), and ADR-004 restated them again and made two of its own quantities depend on them: `T60_max` is *derived* from `m_min`, `T60_min` from `m_max`. docs/phase1-s2-verification-plan.md then recorded the consequence in its "Dependencies not yet decided" section: **all twenty verification cases NS-1…NS-11 and PT-1…PT-9 are blocked on a decided `N` and a decided `mᵢ`** — "until that follow-up ADR is accepted, none of these cases can be run against real numbers."
...
**The S2 verification plan's floor dependency is discharged.** docs/phase1-s2-verification-plan.md's "floor dependency, for every case below without exception" — a decided `N` and a decided `mᵢ` — is now satisfied, and its "at every supported rate" clauses have a concrete two-rate meaning for S2. NS-1's matrix dimension, NS-4's `Γ`, NS-6's per-band realized response, NS-8's `m_max`, and the delay-set floor under every remaining case are all evaluable. **Three quantities still block a subset of cases and are not decided here:** `T60_min` and `T60_max` (NS-5, NS-7, PT-1, PT-2, PT-4, PT-9) and `D_max` (PT-1, PT-2, PT-7). ADR-004 owns the first two as *derivations* that are now evaluable — the closed form and its value at the loosest `δ_margin` are recorded in Rationale — and assigns `D_max` to testing.md's Sonic acceptance gate. Evaluating them is a separate act under ADR-004, not something this ADR performs.
