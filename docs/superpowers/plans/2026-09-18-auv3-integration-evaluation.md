...
- [x] Bound lifecycle/resource/failure handling and the device/OS/sample-rate/block-size matrix; document source-list ownership between portable and Apple builds. **Accepted as [ADR-010](../../decisions/ADR-010-device-lifecycle-matrix.md) (2026-09-18): sample-rate set {48kHz, 44.1kHz}; minimum deployment target iOS/iPadOS 26+ (rolling "current minus one" policy; corrected 2026-09-19 from the 25+ originally recorded — see ADR-010's "Correction note"; re-verify again before implementation).**
