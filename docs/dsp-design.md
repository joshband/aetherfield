...
The late-network topology, its damping filter, denormal mitigation, numerical bounds, full-network initialization/reset/sample-rate lifecycle, the initial parameter surface (Mix, Decay, Damp — units, mappings, smoothing and transport), and the S2 evaluation fixture's line count (`N = 8`), its co-prime delay-length set derived from a 27–81 ms time-domain specification, and its 48/44.1 kHz fixture rates are now **decided but unimplemented** (ADR-002 through ADR-005, [decisions.md](decisions.md)). The product's final `N`, the supported sample-rate matrix, and `T60_min`/`T60_max`/`D_max` stay **DEFERRED**. Modulation and stereo extraction remain hypotheses and are explicitly not authorized by any accepted ADR; ADR-004 records why the accepted parameter set is structurally incapable of expressing modulation. Terra implements further reverb components only under separate authorization; see [phase1-s1-plan.md](phase1-s1-plan.md) for the S1 increment already implemented above.
